...
public class EmpDetails extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        sdesg=(TextView)findViewById(R.id.textView12);
        ldesg=(TextView)findViewById(R.id.textView14);
        mDatabase = FirebaseDatabase.getInstance().getReference();
        posts=ShowAllEmployees.emp.get(ShowAllEmployees.pos)+"";
        mDatabase.child("employees").addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(DataSnapshot dataSnapshot) {
                ...
                for(Map.Entry e:s)
                {
                    ...
                    for(Map.Entry e1:s1)
                    {
                        if(e1.getKey().equals("desig") && e1.getValue().equals(posts+""))
                        {
                            ldesg.setText(posts+"");
                            p1++;
                        }
                    ...
                    if(p1==1)
                    {
                        ...
                        for(Map.Entry e2:s2)
                        {
                            ...
                            if(e2.getKey().equals("username"))
                            {
                                username.setText(e2.getValue()+"");
                            }
                        }
                    }
                }
